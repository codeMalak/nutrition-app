...
import { api } from "../api";
import { Search, Plus, Loader2, Barcode } from "lucide-react";
        ...
        {!loading && results.length > 0 && (
          <div className="space-y-2">
            ...
            {results.map((food) => (
              <div
                key={food.id}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-4 transition-colors duration-200"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {[
                      { label: `${Math.round(scale(food.calories, food.id, food.servingSize))} kcal`, cls: "bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400" },
                      { label: `P ${scale(food.protein, food.id, food.servingSize)}g`,                cls: "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400" },
                      { label: `C ${scale(food.carbs, food.id, food.servingSize)}g`,                  cls: "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" },
                      { label: `F ${scale(food.fats, food.id, food.servingSize)}g`,                   cls: "bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400" },
                    ].map(({ label, cls }) => (
                ...
                <div className="flex items-center gap-2 mt-3">
                  <div className="relative flex-1">
                    <input
                      ...
                      value={grams[food.id] || ""}
                      onChange={(e) =>
                        setGrams({ ...grams, [food.id]: Number(e.target.value) })
                      }
                      className={inputCls}
                    />
                ...
                <div className="mt-2">
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${MEAL_BADGE[selectedMeal]}`}>
                    → {selectedMeal}
                  </span>
                </div>
              </div>
